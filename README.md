...
Create OCI Object Storage Buckets
Backup tablespaces on source database using Oracle-provided Backup Utility.
Create Dynamic Group and Policy to allow Resource Principal access to the object storage buckets.
Provision an Autonomous AI Database by providing migration inputs.
Create Object Storage Buckets
Transportable Tablespaces needs two OCI object storage buckets - one for backups and another for metadata. Create the buckets in your Oracle Storage Cloud Service account. Note the URLs of the buckets as they are needed as inputs for the operation. Use Oracle Cloud Infrastructure Object Storage Native URI Format for the URL.

Example: https://objectstorage.region.oraclecloud.com/n/<namespace-string>/b/<bucket-name>

To make Object Storage URI work, please generate an API signing key. Download the private key .pem file and the API signing key config file with below contents to source database host.
...
Backup Utility Sample Inputs Expand source
...
Transportable Tablespaces functionality will download metadata from OCI Object Storage metadata bucket using OCI Resource Principal. Create Dynamic Group and Policy to allow access to the metadata bucket using the resource principal.

Create a Dynamic Group TTSDynamicGroup with matching rule:
ALL {resource.type = 'autonomousdatabase', resource.compartment.id = 'your_Compartment_OCID'}
Create a Policy using the dynamic group with Policy Statement:
...
Go to OCI Console → Oracle Database → Autonomous AI Database
Click on Create Autonomous Database
Provide all the necessary inputs.
Select database version that is equal to or greater than the source database.
Specify Storage (TB) in Configure the database section to match size of tablespace(s) being transported.
Expand Migration section on the page.
Specify OCI Object Storage URL to the metadata bundle as given by Backup Utility.
Submit Create Autonomous Database.
...
Go to OCI Console → Oracle Database → Autonomous AI Database
Select and click on the database created with the first backup.
Verify Storage in Resource allocation section. Use Manage resource allocation tab to increase storage if needed.
Expand Migration section on the page.
Specify OCI Object Storage URL to the metadata bundle as given by Backup Utility for the increment or final backup.
Click on Save.
...
CLUSTER_MODE : (OPTIONAL INPUT) Accepted values TRUE/true or FALSE/false. If set to TRUE, RMAN allocates backup channels across all open RAC instances (channels_per_host = parallelism / num_rac_instances). If the input parameter is not provided or set to FALSE, all the channels (as specified by parallelism) are allocated only on host where backup tool is run. If CLUSTER_MODE is TRUE, File System should be mounted to all source database host(s). If CLUSTER_MODE is FALSE, File System should be mounted on the host where backup utility is run.
...
Select database version that is equal to or greater than the source database.
Specify Storage (TB) in Configure the database section to match size of tablespace(s) being transported.
Specify Network Access as Private endpoint access only and provide the necessary inputs.
Expand Migration section on the page.
Specify FSS file path to the metadata bundle as given by Backup Utility.
Submit Create Autonomous Database.
The operation will first create the database and then trigger migration operation. For non-incremental operation, migration will perform all the necessary steps and the created database will have the transported tablespaces. In case of incremental operation, this is the first step in the migration process. First backup pieces will be restored to the created database. User has to continue with incremental backup up to the final backup to complete the migration process.
...
Verify Storage in Resource allocation section. Use Manage resource allocation tab to increase storage if needed.
Expand Migration section on the page.
Specify FSS file path to the metadata bundle as given by Backup Utility for the increment or final backup.
Click on Save.
The operation will trigger the transport tablespaces job on the database.
